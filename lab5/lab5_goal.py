...
class Run:
    def __init__(self, factory):
        ...
        self.pidTheta = pid_controller.PIDController(1000, 20, 1, -500, 500, -500, 500)
        self.pidTheta_dist = pid_controller.PIDController(1000, 20, 1, -500, 500, -500, 500)
    ...
    def run(self):
        ...
        print("Initial goal_point " + str(distance))

        result = np.empty((0, 3))
        end_time = self.time.time() + 20
        ...
        while self.time.time() < end_time:
            ...
            if state is not None:
                ...
                # print("reference = " + str(reference) + " measured " + str(measured))
        ...
        plt.title("Angle")
        plt.plot(result[:, 0], result[:, 1], label="desired angle")
        plt.plot(result[:, 0], result[:, 2], label="goal angle")
        plt.grid()
        plt.legend()
        plt.savefig("lab6_angle.png")  # make s ure to not overwrite plots.py

        # plotting for go-to-goal (goal_x, goal_x):
        # plt.figure()
        # f, (ax1) = plt.subplots(1, 2, figsize=(12, 6))
        # ax1.set_title("Angle")
