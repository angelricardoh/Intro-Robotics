...
class Map:
    ...
    def get_random_loc(self):
        while True:
            ...
            diff_x = 15
            diff_y = 15
            if not self.has_obstacle(clamp(x - diff_x,0,self.width-1), y) and \
            not self.has_obstacle(x, clamp(y + diff_y,0,self.height-1)) and not self.has_obstacle(clamp(x + diff_x,0,self.width-1),
                                                                                   clamp(y + diff_y,0,self.height-1)):
                return x, y
